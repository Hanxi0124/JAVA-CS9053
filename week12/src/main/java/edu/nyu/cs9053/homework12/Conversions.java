...
public class Conversions {
    ...
    public static List<String> removeEmptyValuesJava8(List<String> values) {
        ...
        Optional<List<String>> optionalList = Optional.ofNullable(values);
    ...
    public static NavigableSet<String> getUniqueAndNavigableLowerCaseMakeNamesJava8(VehicleLoader vehicleLoader) {
        ...
        Arrays.asList(regionValues)
                .parallelStream()
                .filter(t->vehicleLoader.getVehicleMakesByRegion(t.name()) != null)
                .forEach(t->uniqueVehicleMakes.addAll(vehicleLoader.getVehicleMakesByRegion(t.name())));

        uniqueVehicleMakes.stream()
                .filter(t->t.getName()!=null)
                .forEach(t->navigableMakeNames.add(t.getName().toLowerCase()));
